...
const hardcodedLivestreams = ["listen.moe/stream", "r-a-d.io/main.mp3", "twitch.tv"];
...
const ytdlService = new vdj.YoutubedlService();
function getPlaylist(guild) {
    if (!playlists[guild.id])
        playlists[guild.id] = new DiscordPlaylist(bot.client.voice, guild.id, {
            services: Object.values(services),
            loop: false,
            autoplay: false,
            fallBackService: ytdlService,
            logger: console.log,
        });
...
async function saveCurrentPlaylist(id) {
    ...
    if (playlist.playing && storedPlaylists.playing.indexOf(id) === -1) storedPlaylists.playing.push(id);
    else if (!playlist.playing && storedPlaylists.playing.indexOf(id) > -1) storedPlaylists.playing.splice(storedPlaylists.playing.indexOf(id));
...
async function clearPlaylistSave(id) {
    ...
    if (storedPlaylists.playing.indexOf(id) > -1) storedPlaylists.playing.splice(storedPlaylists.playing.indexOf(id));
...
function loadLastPlaylists() {
    return bot.util
        .load("dj-queues")
        .then((q) => (storedPlaylists = q || { playing: [] }))
        .catch(bot.error);
}
...
function getStreamOptions(id) {
    if (!volumes[id]) setVolume(id, DEFAULT_VOLUME);
    return {
        highWaterMark: 512,
        volume: volumes[id],
        bitrate: "auto",
...
function getSongTime(song, vc) {
    if (vc && vc.dispatcher) return song.seek + vc.dispatcher.streamTime / 1000;
    else return song.seek;
}
...
function setSongDisplayDescription(song, vc) {
    ...
    if (song.info.duration > 0) playtime += "/" + toHHMMSS(song.info.duration);
...
function setSongDisplay(song) {
    ...
    if (song.url.indexOf("youtu") || tags.metadataType === "soundcloud") {
        ...
        if (tags.artist && tags.artist.length) {
            ...
            if (tags.artist.length) artisttext += "ft. " + tags.artist.join(" and ");
            embed.addField("Artist", artisttext, true);
        }
        if (tags.album) {
            var albumtext = tags.album;
            if (tags.albumartist && tags.albumartist.length) albumtext += " ft. " + tags.albumartist.join(", ");
            if (tags.year) albumtext += " (" + tags.year + ")";
...
exports.commands = {
    joinchannel: {
        commands: ["!dj join", "!dj j", "!join", "!summon"],
        ...
        exec: async function (command, message) {
            ...
            try {
                ...
                if (playlist.current) {
                    await startPlaying(playlist, message.member.voice.channel).catch((err) => {
                        bot.error(err);
                        success = false;
                    });
    ...
    addtoqueue: {
        commands: ["!dj queue", "!dj q", "!play"],
        requirements: [bot.requirements.guild, bot.requirements.userInVoice],
        exec: async function (command, message) {
            message.delete({ timeout: DELETE_TIME });
            if ((command.command === "!dj queue" || command.command === "!dj q") && command.arguments.length === 0)
                return exports.commands.showqueue.exec(command, message);
            ...
            if (added.length === 1)
                message.channel
                    .send(message.member.displayName + " added song: " + added[0].title + "\n<" + added[0].info.url + ">")
                    .then((m) => m.delete({ timeout: DELETE_TIME }));
            else {
                message.channel
                    .send(message.member.displayName + " added " + added.length + " songs")
                    .then((m) => m.delete({ timeout: DELETE_TIME }));
            }
        },
    },
    skipcurrent: {
        commands: ["!dj skip", "!dj s", "!skip"],
    ...
    resumemusic: {
        commands: ["!dj resume", "!dj r", "!resume"],
        requirements: [bot.requirements.guild, bot.requirements.botInVoice, bot.requirements.userInVoice],
        exec: async function (command, message) {
            let playlist = getPlaylist(message.guild);
            if (!playlist.current) {
    ...
    showcurrent: {
        commands: ["!dj np", "!dj current", "!dj playing", "!np", "!current"],
        requirements: [bot.requirements.guild, bot.requirements.botInVoice],
        exec: async function (command, message) {
            message.delete({ timeout: DELETE_TIME });
            const playlist = getPlaylist(message.guild);
    ...
    volumechange: {
        commands: ["!dj volume", "!dj vol", "!dj v", "!volume", "!vol"],
        requirements: [bot.requirements.guild, bot.requirements.userInVoice],
        exec: function (command, message) {
